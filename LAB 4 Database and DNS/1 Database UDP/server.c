...
int main(){
	...
	strcpy(students[0].name,"Amy"); strcpy(students[1].name,"Bella"); strcpy(students[2].name,"Cathy");
	strcpy(students[0].add,"Delhi"); strcpy(students[1].add,"Mumbai"); strcpy(students[2].add,"Hyderabad"); 
	strcpy(students[0].sem,"5"); strcpy(students[1].sem,"5"); strcpy(students[2].sem,"5"); 
	strcpy(students[0].dept,"CSE"); strcpy(students[1].dept,"ECE"); strcpy(students[2].dept,"ICT"); 
	strcpy(students[0].sec,"A"); strcpy(students[0].sec,"B"); strcpy(students[0].sec,"B"); 
	...
	while(1){
		...
		recvfrom(s, &choice, sizeof(choice), 0, (struct sockaddr*)&client, &ca);
		if(choice==4)
			break;
		...
		if(pid1==0){	
			...
			if(choice==1){
				...
				sendto(s, &pid, sizeof(pid), 0, (struct sockaddr*)&client,len);
				sendto(s, &name, sizeof(name), 0, (struct sockaddr*)&client,len);
				sendto(s, &add, sizeof(add), 0, (struct sockaddr*)&client,len);
			}		
			exit(0);
		}
	else{
		...
		if(pid2==0){	
			...
			if(choice==2){
				...
				sendto(s, &pid, sizeof(pid), 0, (struct sockaddr*)&client,len);
				sendto(s, &dept, sizeof(dept), 0, (struct sockaddr*)&client,len);
				sendto(s, &sem, sizeof(sem), 0, (struct sockaddr*)&client,len);
				sendto(s, &sec, sizeof(sec), 0, (struct sockaddr*)&client,len);
				sendto(s, &subs, sizeof(subs), 0, (struct sockaddr*)&client,len);
			}
			exit(0);
		}
	else{
		...
		if(pid3==0){	
			...
			if(choice==3){
				...
				sendto(s, &pid, sizeof(pid), 0, (struct sockaddr*)&client,len);
				sendto(s, &marks, sizeof(marks), 0, (struct sockaddr*)&client,len);
			}
			exit(0);
		}
	}
